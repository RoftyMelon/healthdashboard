...
window.BLOODWORK =
{
 ...
 "MARK": [
  ...
  },
  {
   ...
   "note": "An amino acid produced as an intermediate in normal metabolism, then cleared using B12, folate and B6.\n\nSo it works as a functional test of those three: when any of them is short, homocysteine backs up. That makes it more informative than measuring the vitamins directly, since it shows whether the pathway is actually working.\n\nExceptionally sensitive to handling — it keeps rising in the tube if plasma is not separated promptly, which produces falsely high results.\n\nCONFIRM BEFORE INTERPRETING. Red cells keep making homocysteine after the draw, so a delayed spin can add several µmol/L to an otherwise normal sample. A single raised value with no standardised collection behind it is a handling question first and a metabolic one second.\n\nWORK THE VITAMINS IN ORDER. Folate and B12 are the usual causes and the cheapest to check; B6 shows up mainly in the transsulphuration arm. Because the pathway needs all three, the useful sequence is to measure them, correct what is short, and re-test — rather than adding a methyl donor that lowers the number without saying which input was missing.\n\nWHAT ELSE RAISES IT. Impaired kidney function, hypothyroidism, and the MTHFR C677T variant, which reduces enzyme activity and shifts the requirement for folate. Several drugs do too — metformin and proton-pump inhibitors via B12, methotrexate via folate. Creatine synthesis is the body's single largest consumer of methyl groups, so supplemental creatine removes part of that demand and can lower it on its own.\n\nLOWERING IT IS NOT THE SAME AS BENEFIT. B-vitamin trials reduce homocysteine reliably and reduce cardiovascular events barely or not at all — the strongest signal that survives is a modest reduction in stroke. The honest reading is that it is a good marker of B-vitamin adequacy and methylation status, and a poor target in its own right. Treat what it points at.\n\nTHE THRESHOLD IS A CONVENTION. 15 µmol/L is the conventional upper limit, but risk associations are continuous and many laboratories use lower cut-offs. Values in the high teens with normal B-vitamin status and correct handling are usually a set point rather than a finding.",
   "axis": [
    0,
    20
   ],
  ...
  },
  {
   ...
   "note": "A protein that sits on every particle capable of lodging in an artery wall — LDL, VLDL and Lp(a) — exactly one copy each.\n\nSo ApoB counts the particles directly, rather than measuring the cholesterol they happen to be carrying. That matters when the two disagree: two people with identical LDL can carry very different particle numbers, and the particle count is what tracks risk.\n\nIt is also immune to the triglyceride level that distorts calculated LDL.\n\nWHY THE CAUSAL CLAIM IS UNUSUALLY STRONG. Most nutrition and biomarker associations rest on cohorts. This one rests on Mendelian randomisation across dozens of independent variants, each nudging ApoB by a different mechanism, all producing the same proportional change in disease — plus randomised drug trials where statins, ezetimibe and PCSK9 inhibitors lower events in proportion to the ApoB reduction regardless of how they achieve it. Different mechanisms, same result, is what separates causation from correlation here.\n\nIT IS CUMULATIVE, NOT A SNAPSHOT. Arterial damage tracks particle-YEARS, the concentration multiplied by the time spent at it. This is why the same number means something different at 30 than at 60: the younger reading has decades left to accumulate. It is also why a modest lifelong difference outperforms a large late correction, and why the strongest argument for acting early is not that any single reading is dangerous.\n\nWHEN IT DISAGREES WITH LDL, BELIEVE APOB. Discordance is common in insulin resistance, high triglycerides and metabolic syndrome, where particles run small and cholesterol-depleted: LDL-C looks acceptable while particle count is high. The discordant pattern tracks risk with ApoB, not with LDL-C.\n\nWHAT MOVES IT. Saturated fat replaced by unsaturated is the main dietary lever, worth roughly 1.5-2 mg/dL of LDL per 1% of energy exchanged. Soluble fibre and plant sterols each add a further several percent. Body composition and insulin sensitivity matter mostly through triglyceride-rich particles. Beyond that the effective agents are pharmacological.\n\nASSAY NOTE. Measured directly by immunoassay, so it does not inherit the Friedewald assumptions that distort calculated LDL — but methods are standardised to different reference materials, so a change of laboratory can move the number without anything changing in the person.",
   "axis": [
    0,
    160
   ],
  ...
  },
  {
   ...
   "note": "The cholesterol carried on LDL particles, and the main target of lipid treatment.\n\nUsually not measured. Most labs calculate it from total cholesterol, HDL and triglycerides using a formula that assumes a fixed relationship between them. That assumption breaks down in the two places it matters most:\n\n• When LDL is low, the estimate drifts\n• When triglycerides are high, it under-reports\n\nWorth knowing whether a given result was measured directly or calculated.\n\nWHICH FORMULA MATTERS. Friedewald is the old one and fails below about 70 mg/dL and above 400 mg/dL of triglycerides. Martin-Hopkins replaces its fixed factor with one that varies by the sample's own lipid profile and is substantially more accurate at low LDL; Sampson was designed for the high-triglyceride end. A laboratory switching formula moves the number without anything changing in the person, which is why the printed method is worth recording alongside the result.\n\nIT IS A CAUSE, NOT A MARKER. Mendelian randomisation across many independent variants, and randomised trials of drugs working by different mechanisms, both show disease falling in proportion to the LDL reduction rather than to how it was achieved. The exposure that matters is cumulative — concentration multiplied by years — so the same value carries different weight at different ages.\n\nWHY APOB OFTEN BEATS IT. LDL-C measures the cholesterol cargo; ApoB counts the particles. When the two disagree — common with high triglycerides or insulin resistance, where particles run small and cholesterol-poor — risk follows the particle count. A reassuring LDL-C alongside a high ApoB is the pattern this marker misses on its own.\n\nWHAT MOVES IT. Replacing saturated with unsaturated fat is the main dietary lever, roughly 1.5-2 mg/dL per 1% of energy exchanged; 5-10g/day of soluble fibre gives about another 5%; 2g/day of plant sterols about 8-10%. Losing visceral fat works mainly through triglycerides. These are additive but modest — a large gap does not close on diet alone.\n\nTHE THRESHOLDS ARE RISK GRADIENTS, NOT SWITCHES. 130 and 190 mg/dL are conventional decision points, not biological boundaries; risk rises continuously from well below them. A value under a threshold is not evidence of no exposure, only of less.",
   "axis": [
    0,
    190
   ],
  ...
  },
  {
   ...
   "note": "The small slice of testosterone not bound to SHBG or albumin — the portion actually free to enter cells and act.\n\nThis is not measured here. It is calculated from total testosterone, SHBG and albumin using the Vermeulen equation, so it inherits the measurement quirks of all three. That is still the better option: direct free-testosterone immunoassays are notoriously unreliable.\n\nTHE INTERVAL IS THE PRIMARY PUBLISHED ONE. Ho et al., Ann Clin Biochem 2006;43:389-397: 245-785 pmol/L, the 2.5th to 97.5th percentile of the Vermeulen calculation in 126 healthy men aged 20-45 with normal semen analysis — the closest population match available. Laboratories quoting this paper round it slightly (Alfred Health prints 260-740); nearby intervals sit close (Dynacare 196-636 for men under 50, 218-681 for men 20-50).\n\nMIND A LOOSE FLOOR. Widely circulated intervals for this calculation run a third wider than the published one, with almost all the slack at the bottom — a lower bound near 163 pmol/L against a true 2.5th percentile of 245. A genuinely low value reads as normal against a floor set that low, which is the failure mode to watch for whenever an interval arrives without a citation.\n\nMIND THE OTHER SCALE. The dialysis interval on the ftd row is NOT comparable: Vermeulen overestimates dialysis by 20-30%, median ratio 1.19 (Fiers, JCEM 2018), so it maps here at roughly 495-1518 pmol/L — offset upward but overlapping, and disagreeing by more than 1.19 explains because population and SHBG assay differ too. Judge a calculated value against a calculated interval. That leaves a disputed zone of roughly 245-495 pmol/L: inside this interval, below where dialysis maps.\n\nNO OPTIMIZATION TARGET. Calculated free testosterone should be read against a method-matched reference and symptoms, not against a longevity band. An unsourced band is especially costly on this marker: with few datapoints, the band rather than the number decides how a result reads.",
   "axis": [
    40,
    280
   ]
  },
  {
   ...
   "note": "Free testosterone MEASURED, not calculated — the reference method.\n\nSerum sits against a membrane that holds back albumin and SHBG but not testosterone. After 16 hours at 37 degrees the hormone that crossed IS the free fraction, by definition rather than by inference, and the dialysate is read by mass spectrometry.\n\nIT IS A SEPARATE ROW FROM CALCULATED FREE TESTOSTERONE ON PURPOSE. The two are different scales: Vermeulen overestimates dialysis by 20-30%, median ratio 1.19 (Fiers, JCEM 2018). Sharing one row would judge a measurement and an estimate against one interval — the same error that keeps the direct-RIA value out of this file, and the reason creatinine-eGFR and cystatin-C-eGFR are split.\n\nThe interval is Jasuja 2023 (Andrology), standardised equilibrium dialysis with CDC-certified LC-MS/MS: 120-368 pg/mL for men 19-39, median 190, against 66-309 across all healthy non-obese men. Its 10th-90th percentile is descriptive, not an outcome-backed target, so it is not rendered as one.\n\nOrder it whenever a laboratory offers it — availability is the constraint, not a quota. Always beside the same sample's total testosterone, SHBG and albumin: a dialysis value with no calculation next to it teaches nothing about the calculation. REPEATS ARE THE POINT. One pairing gives the direction and rough size of the gap on those assays that day, but it is not a calibration constant — it carries the error of both methods, the offset moves with the assays behind it, and a laboratory changing technique mid-series is something this file has already seen. Several pairings give a spread, and the spread is what says whether an offset can be trusted.",
   "axis": [
    40,
    400
   ]
  },
  {
   ...
   "note": "A liver-made protein that grips testosterone and carries it through the blood. Bound testosterone cannot enter cells, so SHBG effectively decides how much of your total is usable.\n\nThat makes it necessary rather than optional: the same total testosterone means different things at high and low SHBG.\n\nIt also reports on metabolic health — it rises with thyroid hormone and falls with insulin resistance and higher body fat.\n\nNO OPTIMIZATION TARGET, DELIBERATELY. SHBG is barely a lever: about half its variation between people is inherited, and most of the rest tracks liver and metabolic state rather than anything aimed at it directly. Low body fat, insulin sensitivity and thyroid status all raise it, so in a lean and insulin-sensitive person a high-normal SHBG is what those look like from another angle. Every well-evidenced way to move it DOWN is a worse metabolic state: visceral fat, insulin resistance, exogenous androgens. The only benign candidate on this page is boron, which rests on eight men for one week and is parked for exactly that reason. The largest observational signal (UK Biobank, 149,436 men) does put higher SHBG with higher mortality, but it does not survive genetic testing: men with lifelong higher SHBG carry higher TOTAL testosterone, unchanged FREE testosterone, and no excess disease. It still matters as a READING, because a high-normal SHBG is what pushes calculated free testosterone toward the lower part of its interval; it is just not something to move on its own.",
   "axis": [
    10,
    70
   ]
  ...
  },
  {
   ...
   "note": "Cell fragments that form the first plug at a bleeding site — the beginning of a clot.\n\nTracked for bleeding and clotting risk, and as a general check on bone marrow output.\n\nOne common artefact: platelets clump inside the collection tube and the analyser counts a clump as one platelet, so the result reads falsely low. A low count appearing out of nowhere with no symptoms is this until proven otherwise.\n\nNO OPTIMIZATION TARGET, DELIBERATELY. Any band here would point UPWARD, against the direction of the omega-3 starting 1 Aug and the curcumin and AGE garlic parked behind it. Only omega-3 has been shown to move the COUNT in humans — 3.4g/day EPA+DHA for 28 days dropped it 6.3%, which the trialists called clinically insignificant — so the next count should drift down, and a band pointing up would read that as failure. Toward the top of the range a higher count carries more thrombotic risk, not less, so nothing here needs raising — and nothing benign raises it: the agents that work treat immune thrombocytopenia and carry thrombosis and marrow-fibrosis risk.\n\nA count that holds steady in the low-normal zone across years, beside a normal red series, is a set point rather than a signal — the stability is what separates the two. MPV cannot corroborate that, tempting as it is: it is analyser- and delay-dependent enough that values from different laboratories are not comparable, which is the same reason it carries no reference interval.\n\nWhat would change the reading: a downward TREND, several cell lines falling together, or symptoms — easy bruising, petechiae, prolonged bleeding. A single low-normal count with none of those is not a finding.",
   "axis": [
    100,
    450
   ]
  ...
  },
  {
   ...
   "note": "The total number of immune cells in circulation. A broad screen for infection, inflammation and bone-marrow function.\n\nThe total by itself is fairly blunt — nearly all the information is in the breakdown below it, since a high count from neutrophils means something very different from a high count from lymphocytes.\n\nRises briefly with acute stress, adrenaline, and recent hard exercise, none of which involve illness.\n\nNO OPTIMIZATION TARGET, DELIBERATELY. Any band would point UPWARD, and the mortality curve is J-shaped rather than a slope: risk climbs above about 6 and is flat across the low-normal zone in people who are not acutely ill, so a lower edge had nothing to protect against. A count in the lower half of normal is also common in trained people — the evidence is in endurance athletes, though the effect is only about 0.15. What matters is a TREND, or a fall alongside the other cell lines. Judge one by whether it STOPPED: a decline that settles and then holds for years, driven by one line only — neutrophils, with lymphocytes and the red indices unmoved — is a new set point, not a signal, and a fall that also spans a change of laboratory and analyser is partly measurement. The number that decides whether a low white count is worth working up is the absolute neutrophil count, and the threshold is 1500.",
   "axis": [
    2,
    13
   ]
